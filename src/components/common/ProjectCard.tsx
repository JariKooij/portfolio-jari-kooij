        ...
        <Hover>
            <Link href={`/projects/${project.id}`} className="flex grow">
                <article className="flex grow flex-col transition-transform duration-200 hover:scale-[.98]">
                            ...
                            <ul>
                                {project.contributions.map((item) => (
                                    <li key={item} className="relative mb-2 flex last-of-type:mb-0">
                                        <div className="rounded-lg bg-[#daf6ffa1] px-4 py-2 pr-6 text-14px text-black sm:pr-8 sm:text-16px">
                                            {"// " + item}
                                        </div>
                    ...
                    <button className="group flex justify-between rounded-b bg-transparent-white px-6 py-4 uppercase transition-opacity hover:opacity-70  lg:px-8">
